...
function TestimonialCard({ quote, author }: { quote: string; author: string }) {
  return (
    <div className="flex h-[180px] w-[300px] shrink-0 flex-col rounded-2xl bg-neutral-900 p-5 shadow-sm ring-1 ring-neutral-800 sm:h-[200px] sm:w-[320px]">
      <p className="min-h-0 flex-1 overflow-hidden text-sm leading-relaxed text-white/80 line-clamp-4 sm:text-base">&ldquo;{quote}&rdquo;</p>
      <p className="mt-4 shrink-0 font-medium text-white">{author}</p>
    </div>
  );
}
...
export function Testimonials() {
  return (
    <section className="bg-black px-4 pt-10 pb-10 sm:px-6 sm:pt-12 sm:pb-12">
      <div className="mx-auto max-w-5xl">
        ...
        <div className="relative mt-12 overflow-hidden">
          <div
            className="pointer-events-none absolute left-0 top-0 z-10 h-full w-16 bg-gradient-to-r from-black to-transparent sm:w-20"
            aria-hidden
          />
          <div
            className="pointer-events-none absolute right-0 top-0 z-10 h-full w-16 bg-gradient-to-l from-black to-transparent sm:w-20"
            aria-hidden
          />
